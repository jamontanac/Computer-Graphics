...
void simulate( int msecs )
{
  ...
  float t = float( std::chrono::duration_cast< std::chrono::nanoseconds >( t1 - t0 ).count( ) ) * 1e-3;

  projectile_pos[ 0 ] = ( projectile_vel[ 0 ] * t ) + projectile_x0[ 0 ];
  projectile_pos[ 1 ] = gravity_magnitude * t * t + ( projectile_vel[ 1 ] * t ) + projectile_x0[ 1 ];

  glutTimerFunc( 10, simulate, 0 );
  */
}
...
void click( int button, int state, int x, int y )
{
  ...
  {
    if( current_state == S_localizing )
    {
      current_state = S_aiming;
      projectile_vel[ 0 ] = projectile_pos[ 0 ];
      projectile_vel[ 1 ] = projectile_pos[ 1 ];
    }
    else if( current_state == S_aiming )
    {
      current_state = S_simulating;

      projectile_vel[ 0 ] -= projectile_pos[ 0 ];
      projectile_vel[ 1 ] -= projectile_pos[ 1 ];
      projectile_x0[ 0 ] = projectile_pos[ 0 ];
      projectile_x0[ 1 ] = projectile_pos[ 1 ];
...
void passive_move( int i, int j )
{
  ...
  {
    float x, y;
    pickPoint( x, y, InvPrj, i, j );
    if(
      x - projectile_radius >= world_P0[ 0 ]
      &&
      x + projectile_radius <= world_P1[ 0 ]
      &&
      y - projectile_radius >= world_P0[ 1 ]
      &&
      y + projectile_radius <= world_P1[ 1 ]
      )
    {
      projectile_pos[ 0 ] = x;
      projectile_pos[ 1 ] = y;
    ...
    pickPoint( projectile_vel[ 0 ], projectile_vel[ 1 ], InvPrj, i, j );
    */
}
...
void draw( )
{
  ...
  */
  ...
  /*
    ...
    if( current_state == S_aiming )
    {
    glColor3f( 1, 0, 0 );
    glBegin( GL_LINES );
    glVertex2f( projectile_pos[ 0 ], projectile_pos[ 1 ] );
    glVertex2f( projectile_vel[ 0 ], projectile_vel[ 1 ] );
    glEnd( );
    } // end if

    // Projectile
    glTranslatef( projectile_pos[ 0 ], projectile_pos[ 1 ], 0.0 );
    glScalef( projectile_radius, projectile_radius, 1.0 );
    glColor3f( 0, 1, 0 );
    circle( GL_LINE_LOOP, 8 );
  */
